...
# Check and stop service on port 5353
if netstat -tuln | grep ":5353 " > /dev/null; then
  echo "Stopping service on port 5353"
  service=$(netstat -tuln | grep ":5353 " | awk '{print $7}')
  pid=$(echo $service | cut -d'/' -f1)
  kill -9 $pid
  echo "Service on port 5353 stopped."
else
  echo "No service found on port 5353"
fi
